...
def get_cpp_parse_from_rapidjson_helper_int(t, member, indent):
    return '{i}if(iter->value.IsInt64()==false) return false; \n' \
           '{i}else {{\n' \
           '{i}  int64 gafv = iter->value.GetInt64();\n' \
           '{i}  if(gafv < numeric_limits<{t}>::min()) return false;\n' \
           '{i}  if(gafv > numeric_limits<{t}>::max()) return false;\n' \
    ...
    with open(os.path.join(out_dir, name+'.h'), 'w', encoding='utf-8') as out:
        ...
        if write_json:
            if header_only:
                ...
                source.append('#include <limits>\n')
                source.append('#include "rapidjson/document.h"\n')
                source.append('\n')
        ...
        for s in f.structs:
            ...
            if write_json:
                ...
                source.append('  Value::ConstMemberIterator itr;\n');
                for m in s.members:
                    source.append('  itr = document.FindMember("{n}");\n'.format(n=m.name));
                    source.append('  if(itr != document.MemberEnd()) {\n');
